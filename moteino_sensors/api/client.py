...
class MgwApiClient(ApiClient):

  def get_status(self):
    return self.request('/action/status/')

  def _set_armed_status(self, status):
    params = {'armed': status}
    return self.request('/action/status', method='POST', json=params)

  def arm_alarm(self):
    return self._set_armed_status(status=1)

  def unarm_alarm(self):
    return self._set_armed_status(status=0)
